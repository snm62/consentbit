...
const CMP_SCRIPT = `(async function () {
  ...
  async function loadConsentState() {
    if (isLoadingState) {
      ...
      const ccpaConsentGiven = localStorage.getItem("ccpa-consent-given");
      ...
      if (window.currentBannerType === 'GDPR') {
      ...
      if (window.currentBannerType === 'GDPR' && !initialBlockingEnabled) {
        unblockScripts();
    }
              ...
              function updateGAConsent() {
                  if (typeof gtag === "function") {
                      ...
                      gtag('consent', 'update', {
                          ...
                          // e.g., 'personalization_storage': normalizedPrefs.personalization ? 'granted' : 'denied', 
...
function updateConsentState(preferences) {
  ...
  const isRejection = preferences.ccpa?.doNotShare  || 
  ...
  if (preferences.ccpa?.doNotShare) {
    blockMarketingScripts();
    blockPersonalizationScripts();
    blockAnalyticsScripts();
  } else {
...
if (newToggleConsentButton) {
  ...
  if (saveCCPAPreferencesButton) {
    saveCCPAPreferencesButton.addEventListener("click", async function(e) {
      ...
      const preferences = {
        necessary: true, // Always true
        marketing: consentState.marketing || false, // Preserve existing GDPR state
        personalization: consentState.personalization || false, // Preserve existing GDPR state
        analytics: consentState.analytics || false, // Preserve existing GDPR state
        ccpa: {
            doNotShare: doNotShare // Correctly nested
        }
      };
      await updateConsentState(preferences);
